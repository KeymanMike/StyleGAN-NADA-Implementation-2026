...
### Анализ обучения
### Изменение весов и градиент по слоям
<img width="1788" height="3190" alt="image" src="https://github.com/user-attachments/assets/a38e4d79-d6ae-4d30-916b-2576590e64bb" />

**Наблюдения:**

- **Слои 8×8 – 64×64 изменяются активнее всего** (в среднем ~0.08). Эти слои формируют базовые черты лица, цветовую палитру и освещение — ключевые атрибуты аниме-стиля.
  
- **Слой 1024×1024 меняется слабее и затухает** (с 0.028 до 0.012). В аниме-домене нет необходимости в реалистичных текстурах (поры, морщины), поэтому модель быстро стабилизирует этот уровень.
  
- **Общая картина подтверждает эффективность NADA:** стиль адаптируется в первую очередь на средних и высоких разрешениях, а финальная детализация работает как «шлифовка», не перегружая модель.

**Вывод:** Для адаптации «фото → аниме» критически важны слои 8×8 – 64×64. Разморозка начиная с 32×32 даёт наилучший баланс между изменением стиля и сохранением структуры.


### Графики лоссов и нормы градиента и изменения весов для текста
<img width="2790" height="1489" alt="image" src="https://github.com/user-attachments/assets/a0b3f70f-5fe8-4fcf-afc8-68914d91db79" />



### Графики лоссов и нормы градиента и изменения весов для глубины
